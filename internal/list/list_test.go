...
func TestList(t *testing.T) {
	...
	// Single element list
	single := New[string]()
	checkListPointers(t, single, []*Element[string]{})

	e := single.PushFront("a")
	checkListPointers(t, single, []*Element[string]{e})
	single.MoveToFront(e)
	checkListPointers(t, single, []*Element[string]{e})
	single.MoveToBack(e)
	checkListPointers(t, single, []*Element[string]{e})
	single.Remove(e)
	...
	e1 := l.PushFront(1)
	e3 := l.PushBack(3)
	e4 := l.PushBack(4) // type is now constrained
	...
	// 8 here vs 4 in non-generic source because e4=4
	if sum != 8 {
